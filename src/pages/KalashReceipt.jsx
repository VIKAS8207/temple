...
export default function KalashReceipt() {
  ...
  if (!receiptData) {
    return <Navigate to="/kalash" replace />;
  }
  ...
  const downloadPDF = async () => {
    ...
    try {
      const receiptElement = document.getElementById('kalash-receipt-container');
      ...
      pdf.save(`${receiptData.bookingId}_Kalash_Receipt.pdf`);
      ...
      >
        <div className="border border-stone-300 m-2 p-6 sm:p-12 relative bg-white">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-12 opacity-10 pointer-events-none z-0 select-none">
            <span className="text-[90px] md:text-[120px] font-black text-green-600 uppercase tracking-widest border-8 border-green-600 p-4 rounded-xl">PAID</span>
          </div>
          ...
          <div className="relative z-10">
            ...
            <div className="text-center mb-8 pb-6 border-b-2 border-stone-800">
              ...
              <p className="text-sm font-medium text-stone-500 italic bg-amber-50 inline-block px-4 py-1 rounded-full border border-amber-200">
                {t('kalash.receiptThanks', 'आपके भक्तिपूर्ण समर्पण के लिए धन्यवाद!')}
              </p>
            </div>
            ...
            <div className="flex justify-between items-center bg-stone-50 border border-stone-200 py-3 px-6 rounded mb-8">
              <div>
                <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-0.5">{t('kalash.receiptId', 'बुकिंग आईडी')}</p>
                <p className="font-mono text-lg font-bold text-stone-800 tracking-wider">{receiptData.bookingId}</p>
              </div>
              <div className="text-right">
                <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-0.5">{t('kalash.receiptDate', 'दिनांक')}</p>
                <p className="font-bold text-stone-800">{receiptData.date}</p>
              </div>
            </div>

            {/* Seva Details */}
            <div className="mb-8 p-4 bg-amber-50/50 border border-amber-200 rounded-lg flex justify-between items-center">
              <div>
                <p className="text-amber-800 text-[10px] font-bold uppercase tracking-widest mb-1">{t('kalash.sevaDetails', 'सेवा विवरण')}</p>
                <p className="font-bold text-amber-900 text-lg leading-relaxed">{receiptData.package.festival}</p>
              </div>
              <div className="text-right">
                <span className="text-orange-950 font-aparajita text-xl font-bold bg-white px-3 py-1 rounded-md border border-amber-100">
                  {t(`kalash.${receiptData.package.type}`)}
                </span>
              </div>
            </div>
              ...
              {receiptData.isAnonymous ? (
                <div className="p-8 bg-stone-50 text-center flex flex-col items-center justify-center">
                  ...
                  <h3 className="text-stone-700 font-bold text-xl uppercase tracking-widest mb-2">
                    {t('kalash.anonymousDonor', 'गुमनाम भक्त')}
                  </h3>
                  <p className="text-stone-500 text-sm">
                    {t('kalash.anonymousMsg', 'भक्त ने अपनी पहचान गुप्त रखने का विकल्प चुना है। व्यक्तिगत विवरण सुरक्षित रखा गया है।')}
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="p-4 bg-white">
                    <p className="text-stone-400 text-[10px] font-bold uppercase tracking-wider mb-1">{t('kalash.address', 'पता')}</p>
            ...
            <div className="flex justify-between items-end border-t-2 border-stone-800 pt-6 mt-8">
              <div>
                <p className="text-stone-800 text-sm font-bold uppercase tracking-widest mb-1">{t('kalash.receiptAmount', 'जमा राशि')}</p>
                <p className="text-stone-500 text-xs italic">Received with thanks</p>
              </div>
              <p className="text-4xl font-black text-[#8B3A2B]">₹{receiptData.package.price}</p>
            </div>
