// src/pages/LuckyDrawReceipt.jsx
import React, { useEffect, useState } from 'react';
import { useLocation, Link, Navigate } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
// FIX 1: Use Named Import for jsPDF (This was causing the crash!)
import { jsPDF } from 'jspdf'; 
import html2canvas from 'html2canvas';

export default function LuckyDrawReceipt() {
  ...
  // FIX 2: Custom Notification State (No more system alerts!)
  ...
  // If someone tries to access this page directly without submitting the form, send them back
  if (!receiptData) {
    return <Navigate to="/lucky-draw" replace />;
  }
  ...
  const downloadPDF = async () => {
    ...
    setNotification({ show: false, message: '', type: '' }); // Clear old alerts
    ...
    try {
      ...
      pdf.save(`${receiptData.couponId}_Receipt.pdf`);

      // Show Custom Success Notification
      showToast(t('luckyNew.pdfSuccess', 'रसीद सफलतापूर्वक डाउनलोड हो गई! (Downloaded Successfully)'), 'success');

    } catch (error) {
      console.error("Error generating PDF:", error);
      // Show Custom Error Notification
      showToast(t('luckyNew.pdfError', 'PDF जनरेट करने में विफल। कृपया पुनः प्रयास करें। (Failed to generate PDF)'), 'error');
    } finally {
      ...
      >
        ...
        <div className="border border-stone-300 m-2 p-6 sm:p-12 relative bg-white">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-12 opacity-10 pointer-events-none z-0 select-none">
            <span className="text-[120px] font-black text-green-600 uppercase tracking-widest border-8 border-green-600 p-4 rounded-xl">Paid</span>
          </div>
          ...
          <div className="relative z-10">
            ...
            <div className="text-center mb-8 pb-6 border-b-2 border-stone-800">
              ...
              <h1 className="text-3xl sm:text-4xl font-rozha text-[#8B3A2B] mb-2 uppercase tracking-wide">
                {t('luckyNew.receiptTitle', 'सहयोग रसीद')}
              </h1>
              <p className="text-stone-600 font-medium tracking-widest uppercase text-xs">
            ...
            <div className="flex justify-between items-center bg-stone-50 border border-stone-200 py-3 px-6 rounded mb-8">
              <div>
                <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-0.5">{t('luckyNew.receiptId', 'कूपन आईडी')}</p>
                <p className="font-mono text-lg font-bold text-stone-800 tracking-wider">{receiptData.couponId}</p>
              </div>
              <div className="text-right">
                <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-0.5">{t('luckyNew.receiptDate', 'दिनांक')}</p>
            ...
            <div className="border border-stone-200 rounded">
              ...
              <div className="p-4 bg-white">
                <p className="text-stone-400 text-[10px] font-bold uppercase tracking-wider mb-1">{t('luckyNew.receiptAddress', 'पता')}</p>
